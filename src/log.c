...
#ifndef useSprintf
static uint16_t
strcopy(char* s, const char* format, uint16_t counter) {
    uint16_t retN = 0;
...
static uint8_t
u_to_str(uint32_t num, char* buf) {
    uint8_t len = 0;
    do {
        buf[len++] = (num % 10) + '0';
        num /= 10;
...
static int
_vsnprintf_nano(char* s, uint16_t n, const char* format, va_list ap) {
    uint16_t counter = n;
    uint8_t width = 0;
...
int
snprintf_nano(char* s, uint16_t n, const char* format, ...) {
    va_list ap;
    va_start(ap, format);
...
#ifdef useSprintf
    counter += sprintf(TxBuffer1, timeFormat, (uint16_t)(millis() / 3600000), (uint16_t)(millis() % 3600000 / 60000),
                       (uint16_t)(millis() % 60000 / 1000), (uint16_t)(millis() % 1000));
    counter += vsprintf((char*)(TxBuffer1 + 14), pStr, ap) + 14;
#else
    counter += snprintf_nano(TxBuffer1, 16, timeFormat, (uint32_t)(millis() / 3600000),
                             (uint32_t)(millis() % 3600000 / 60000), (uint32_t)(millis() % 60000 / 1000),
...
void
UART1_TX_IRQHandler() __interrupt(17) {
